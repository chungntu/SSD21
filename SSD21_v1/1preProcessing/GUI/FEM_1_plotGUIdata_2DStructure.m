...
%--------------------------------------------------------------------------
function FEM_1_plotGUIdata_2DStructure(in_data)
...
for ELEMENT=1:size(in_data.EL)
    node1 = find(in_data.ND(:,1)==in_data.EL(ELEMENT,3));
    node2 = find(in_data.ND(:,1)==in_data.EL(ELEMENT,4));
    % PLOT STRUCTURE
    plot([in_data.ND(node1,2),in_data.ND(node2,2)],...
        [in_data.ND(node1,3),in_data.ND(node2,3)],'k-','LineWidth',2);
    % PLOT NODES NUMBER ---------------------------------------------------
    if size(in_data.EL)<250
        nodeLabel = text(in_data.ND(node1,2)+shift,in_data.ND(node1,3)+shift,...
            num2str(node1)); set_font1(nodeLabel);
        nodeLabel = text(in_data.ND(node2,2)+shift,in_data.ND(node2,3)+shift,...
            num2str(node2));
        set_font1(nodeLabel);
        % PLOT ELEMENTS NUMBER ------------------------------------------------
        x1 = (in_data.ND(node2,2)+in_data.ND(node1,2))/2 + shift;
        y1 = (in_data.ND(node2,3)+in_data.ND(node1,3))/2 + shift;
        frameLabel = text(x1,y1,num2str(in_data.EL(ELEMENT,1)));
        set_font2(frameLabel);
    end
    % PLOT UNIFORM LOADS
    plotUniformLoad(in_data,lab,ELEMENT)
end
% PLOT CONCENTRATED LOADS
plotConcentratedLoad(in_data,lab); hold on
% PLOT RESTRAINTS
...
if ~isempty(in_data.MASS_)
    ...
    % PLOT RESTRAINTS
    plotRestraint(in_data,in_data.ND,lab); hold on
    plotMovingLumpMass(in_data,in_data.ND,lab); hold off
...
% LOADS IN Y-DIRECTION USE DOT LINE
if  isfield(in_data,'qy') && in_data.qy(ELEMENT)~=0
    plotLoc_qy = [0:0.05:L]';
    if in_data.qy(ELEMENT)> 0; laby = -lab; else; laby = lab; end
    plotLoc_qy_T = [plotLoc_qy,ones(length(plotLoc_qy),1)*laby*0.3]*T;
    plot(x1+plotLoc_qy_T(:,1),y1+plotLoc_qy_T(:,2),'b:','Linewidth',2);
end
end
