...
switch EL_TYPE
    case {0,1,2,333} % 2D Frame
        for CONSTRAINT = 1:size(in_data.CON_,1)
            NODE = find(in_data.ND(:,1)==in_data.CON_(CONSTRAINT,1));
            % PIN CONSTRAINT
            if (in_data.CON_(CONSTRAINT,2)==0 && in_data.CON_(CONSTRAINT,3)==0 && in_data.CON_(CONSTRAINT,4)~=0)
