function FEM_0_buckling(in_data,resp,model,opt)
[~,EL_TYPE,~] = elemType(in_data);
if EL_TYPE == 0 || EL_TYPE == 1 || EL_TYPE == 2 || EL_TYPE == 333
    if opt.static % MUST RUN STATIC ANALYSIS BEFORE BUCKLING ANALYSIS
        if opt.buckling
            if ~isempty(in_data.MASS_)
                showMode = min(size(in_data.MASS_,1),6);
            else
                showMode = 6;
            end
            Np              = full(resp.static.Np);
            L               = model.freeDOFs;
