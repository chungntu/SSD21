...
%--------------------------------------------------------------------------
function [Np,Vp,Mp] = FEM_3_static_plot_NVM(in_data,resp,opt)
% CALCULATE N,V,M
xL  = (0:0.05:1)'; N  = length(xL);
Np  = zeros(N,size(in_data.EL,1));
Vp  = zeros(N,size(in_data.EL,1));
Mp  = zeros(N,size(in_data.EL,1));
for ELEMENT = 1:size(in_data.EL,1)
    [Np(:,ELEMENT),Vp(:,ELEMENT),Mp(:,ELEMENT)] = calculateNVM(in_data,resp,xL,ELEMENT);
end
...
if opt.showFig
    ...
    hold off
    % export_fig momentDiagram -transparent
...
function [Np,Vp,Mp] = calculateNVM(in_data,resp,xL,ELEMENT)
N  = length(xL);
node1 = in_data.EL(ELEMENT,3);
node2 = in_data.EL(ELEMENT,4);
xNode1 = in_data.ND(node1,2);  yNode1 = in_data.ND(node1,3);
...
L = sqrt((xNode2-xNode1)^2 + (yNode2-yNode1)^2);
cosa = (xNode2-xNode1)/L; sina = (yNode2-yNode1)/L;
E = in_data.EL(ELEMENT,5); A = in_data.EL(ELEMENT,6);
I = in_data.EL(ELEMENT,7); rho= in_data.EL(ELEMENT,8);
...
qz = -in_data.qy(ELEMENT)*(cosa)+in_data.qx(ELEMENT)*(sina);
qx = -in_data.qy(ELEMENT)*(sina)+in_data.qx(ELEMENT)*(cosa);
Np = Ne(1) + (0:N-1)'*(Ne(2)-Ne(1))/(N-1) - L*qx/2 + xL*L*qx;
Vp = Ve(1) + (0:N-1)'*(Ve(2)-Ve(1))/(N-1) - L*qz/2 + xL*L*qz;
switch in_data.EL(ELEMENT,2)
    case {0}
        Mp = Me(1) + (0:N-1)'*(Me(2)-Me(1))/(N-1)...
            - ( qz*L^2/12 -  qz*L/2*(xL*L)  + qz*(xL*L).^2/2);
    case{1}
        Mp = Me(1) + (0:N-1)'*(Me(2)-Me(1))/(N-1)...
            - (qz*L^2/8 - 5*qz*L/8*(xL*L) + qz*(xL*L).^2/2);
    case {2}
        Mp = Me(1) + (0:N-1)'*(Me(2)-Me(1))/(N-1)...
            - (-3*qz*L/8*(xL*L) + qz*(xL*L).^2/2);
    case {333}
        Mp = -(-qz*L/2*(xL*L) + qz*(xL*L).^2/2);
end
end
